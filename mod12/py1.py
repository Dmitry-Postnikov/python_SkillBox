...
def download_characters_characteristics_pool():
    characters_characteristics = []
    character_characteristics_urls = [f'https://swapi.dev/api/people/{i}/' for i in range(1, 21)]
    with Pool() as pool:
        results = pool.map(download_character_characteristics_data, character_characteristics_urls)
...
def main():
    ...
    measure_time_pool(download_characters_characteristics_pool)
